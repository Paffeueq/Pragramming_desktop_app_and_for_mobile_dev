...
✓ Wiadomość przetworzona pomyślnie
✓ Dane zapisane w bazie danych
✓ Timestamp przetworzenia zarejestrowany
\end{verbatim}
...
✓ Nie ma zalegających wiadomości
\end{verbatim}

\subsection{Porównanie: SQLite vs Azure Storage Queue}

\begin{center}
\begin{tabular}{|l|l|l|}
\hline
\textbf{Aspekt} & \textbf{Lokalne rozwiązanie} & \textbf{Azure Storage Queue} \\
\hline
Przechowywanie & Plik lokalny lub w pamięci & Cloud (z6storage) \\
\hline
Trwałość & Efemeryczna & Persystentna (replikowana) \\
\hline
Skalowanie & Ograniczone & Automatyczne \\
\hline
Dostępność & Lokalna & 24/7 w chmurze \\
\hline
Poison queue & Brak wsparcia & Auto-tworzona \\
\hline
Monitoring & Brak & Azure Portal + Application Insights \\
\hline
Cost & Darmowe & \$0.01 za milion operacji \\
\hline
\end{tabular}
\end{center}

\subsection{Architektura – Diagram przepływu}

\begin{verbatim}
┌─────────────────┐
│  HTTP Client    │
└────────┬────────┘
         │ POST /api/SendToQueue
         ▼
┌──────────────────────────────┐
│  SendToQueue Endpoint        │
...
\begin{itemize}
	\item \textbf{Azure Storage Queue} zapewnia niezawodne, skalowalne przetwarzanie asynchroniczne
	\item \textbf{QueueTrigger} automatycznie odbiera i przetwarza wiadomości bez interwencji
	\item \textbf{Persistence} -- wiadomości przechowywane w chmurze Azure, dostępne 24/7
	\item \textbf{Audit trail} -- każda wiadomość rejestrowana w bazie danych z timestampem
	\item \textbf{Poison queue} -- automatyczna obsługa wiadomości błędnych
	\item \textbf{Skalowanie} -- brak obaw o wydajność przy wzroście liczby wiadomości
	\item \textbf{Dane rzeczywiste} -- system wdrożony na faktycznym Azure Storage Account (\texttt{z6storage})
\end{itemize}
